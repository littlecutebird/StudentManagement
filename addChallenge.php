...
if (empty($upload_err) && isset($_POST["submitChallenge"])) {
    // Update database homework: id, teacherId, title, description, filePath, modified_time, deadline
    $insert_query = "INSERT INTO challenge SET teacherId = {$_SESSION["id"]}, title = ?, description = ?, filePath = ?, modified_time = now(), deadline = ?";
    if ($stmt = mysqli_prepare($db_connection, $insert_query)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                $upload_err = "Upload challenge success!";
            }
            else {
                echo "Cannot execute insert SQL query";
                exit;
            }
    ...
    <body>
        ...
        <div class='container'>
            <form action='' method='post' enctype='multipart/form-data'>
                ...
                <div class='form-group'>
                    <button type="submit" class="btn btn-success" value="Upload File" name="submitChallenge">Add new challenge</button>  
                    <button class='btn btn-warning' type='reset'>Reset</button>
                    <a class='btn btn-primary' href='listChallenge.php'>Cancel</a>
                </div>
